...
#1:
df_filtered_year_2000 = data[data['year'] == 2000] # Filtrando os dados do ano de 2000
...
df_filtered_year_plot = df_filtered_year[(df_filtered_year['wind_energy_per_capita'] >= lower_limit) & (df_filtered_year['wind_energy_per_capita'] <= upper_limit)]
# Criando o ColumnDataSource
wind_scatter_sem_outlier = ColumnDataSource(df_filtered_year_plot)
...
# Criando o ColumnDataSource
wind_scatter_com_outlier = ColumnDataSource(df_filtered_year)
# Aplicando regressão linear usando scipy
slope, intercept, r_value, p_value, std_err = stats.linregress(df_filtered_year['wind_elec_per_capita'], df_filtered_year['wind_energy_per_capita'])
...
x_line = np.array([df_filtered_year['wind_elec_per_capita'].min(), df_filtered_year['wind_elec_per_capita'].max()])
y_line = slope * x_line + intercept
regression_line_new_scale = Slope(gradient=slope, y_intercept=intercept, line_color='#8A5556', line_width=2)
