...
oil_text_2 = Div(text = """</br> <p style="font-size:16px;">O objetivo dessa visualização é exibir a evolução da produção de petróleo ao longo dos anos, olhando mais especificamente para as regiões do mundo que mais concentram essa produção: Oriente Médio, América do Norte, África, Ásia e Américas Central e do Sul. Dessa forma, o tipo de gráfico mais adequado para essa situação era o gráfico de linhas.</p> </br>

<p style="font-size:16px;">A primeira coisa a se fazer após a plotagem das linhas era sua diferenciação, já que estavam todas idênticas. Para isso, foi utilizado o atributo de cor, selecionando-se cores que se destacassem umas das outras: amarelo, laranja, verde, azul e vermelho. Preferi optar por cores mais primárias e "distantes" para que não acontecesse qualquer confusão com relação a qual linha representa qual região. </br>
...
<p style="font-size:16px;">Após isso, comecei a manipular os eixos. Exclui os ticks secundários pois considerava-os irrelevantes, de tal forma que acabavam sendo apenas um elemento de poluição. Inseri os títulos dos eixos de modo a ficarem precisos e concisos, aumentando um pouco o tamanho de suas fontes e mudando apenas seus estilos (de itálico para normal) e suas fontes para Arial.</p> </br>

<p style="font-size:16px;">Feito isso, criei o título da visualização, indicando todos os dados presentes no gráfico: a produção de petróleo, os anos e as regiões. Reduzi a fonte para que ele coubesse na parte de cima do gráfico e centralizei-o com a visualização.</p> </br>

<p style="font-size:16px;">Com relação ao fundo, considerei como melhor opção manter a cor branca, apenas alterando levemente a opacidade dos eixos, para que não ficassem tão destacados e incomodassem a visualização dos dados.</p> </br>

<p style="font-size:16px;">Trabalhando agora com as ferramentas, depois de uma análise individual das possibilidades, avaliei como realmente úteis apenas as ferramentas "pan", para movimentar o gráfico, "wheel_zoom", para dar zoom, "reset", para retornar à posição inicial, e "save", para baixar uma versão da visualização. Além disso, desativei a logo do Bokeh da toolbar, pois não era necessária aqui, e habilitei a função "autohide", para que ela só fosse exibida quando solicitada.</p> </br>

<p style="font-size:16px;">Por fim, adicionei anotações em caixa e em texto para destacar a forte queda na produção na década de 80, que afetou principalmente o Oriente Médio. Isso se deu devido à explosão da guerra entre o Iraque e o Irã, que gerou instabilidades na região. Assim, escolhi a cor vermelha para esses destaques para expressar a negatividade do fato.</p> </br>

<h2>Gráfico "Consumo de petróleo x População em 2019"</h2>""")


oil_text_3 = Div(text = """</br> <p style="font-size:16px;">O objetivo dessa segunda visualização é mostrar a correlação existente entre a população dos países e seu consumo de petróleo. Para isso, é claro, foi utilizado um gráfico de dispersão.</p> </br>

<p style="font-size:16px;">Antes de realizar a plotagem dos dados, separei os registros de acordo com sua média de consumo por habitante: os países cuja média estava acima da mundial e os países nos quais ela estava abaixo. Dessa forma, pude agrupá-los por meio da cor, caracterizando-os como vermelhos e verdes, respectivamente. Tentei fazer com que essa média fosse representada por meio de um degradê de cores, mas a diferença entre esses valores era tão pequena que a variação dessas cores se tornou quase inexistente. Assim, preferi manter o modelo de 2 categorias.</br>
...
<p style="font-size:16px;">Trabalhando agora com os eixos, primeiramente, configurei-os para a escala logarítmica, pois os dados estavam ficando muito agrupados na parte inferior esquerda. Assim, é possível exibir melhor sua distribuição. </br>
...
<p style="font-size:16px;">Após isso, adicionei e formatei o título, deixando-o com as mesmas configurações que o do primeiro gráfico e escrevendo-o de uma maneira que resumisse bem as informações presentes na visualização.</p> </br>

<p style="font-size:16px;">A respeito do fundo, novamente, optei por manter a cor branca padrão, pois as cores do gráfico já estavam bem destacadas, fazendo com que uma outra cor de fundo o deixasse muito poluído visualmente. Assim, a única coisa que modifiquei foi a opacidade do grid, tornando-o um pouco mais sutil.</p> </br>
...
<p style="font-size:16px;">Por fim, para montar o gridplot, optei por colocar os três gráficos um acima do outro, de modo a alinhar as marcações do eixo x. Fiz isso para que fossem mais facilmente comparáveis as informações ao longo dos <a href="../drafts/easter_egg.html">anos</a>.</p> </br>
