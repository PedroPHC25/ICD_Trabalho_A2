...
Em seguida, é claro que era necessária uma legenda para essas cores. Assim, criei-a na parte superior esquerda do gráfico para que não encobrisse parte dos dados.</p>

<p>Após isso, comecei a manipular os eixos. Exclui os ticks secundários pois considerava-os irrelevantes, de tal forma que acabavam sendo apenas um elemento de poluição. Inseri seus títulos de modo a ficarem precisos e concisos, e optei por não alterar seus tamanhos, mudando apenas seus estilos (de itálico para normal) e suas fontes para Arial.</p>

<p>Feito isso, criei o título da visualização, indicando todos os dados presentes no gráfico: a produção de petróleo, os anos e as regiões. Reduzi a fonte para que ele coubesse na parte de cima do gráfico e centralizei-o com a visualização.</p>
...
Após isso, novamente criei a legenda para os dados, indicando que os pontos vermelhos representavam os países com média de consumo de petróleo acima da mundial e que os verdes representavam os cuja média estava abaixo da global.</p>
...
Com relação aos seus títulos, da mesma forma que anteriormente, criei-os de uma forma curta e precisa, apenas modificando sua fonte e seu estilo.</p>

<p>Após isso, adicionei e formatei o título, deixando-o com as mesmas configurações que o do primeiro gráfico e escrevendo-o de uma maneira que resumisse bem as informações presentes na visualização.</p>

<p>A respeito do fundo, novamente, optei por manter a cor branca padrão, pois as cores do gráfico já estavam bem destacadas, fazendo com que uma outra cor de fundo o deixasse muito poluído visualmente. Assim, a única coisa que modifiquei foi a opacidade do grid, tornando-o um pouco mais sutil.</p>

<p>Trabalhando agora com a toolbar, assim como na primeira visualização, ativei a funcionalidade "autohide" e ocultei a logo do Bokeh. Já com relação às ferramentas em si, mantive as mesmas que anteriormente, apenas adicionando a ferramenta "hover" e configurando-a para que exibisse o país correspondente a cada ponto quando o mouse fosse passado por ele. Como antes eu não tinha nenhum indicativo de quais eram aqueles países, considerei bastante útil essa informação adicional, que agrega valor aos dados do gráfico.</p>

<p>Por fim, decidi fazer uma anotação simples, apenas destacando a proporção direta entre a população dos países e seu consumo de petróleo. Para que esse texto não ficasse tão destacado, atribui a ele uma cor cinza e tornei-o um pouco transparente, de tal forma que ele não "ofuscasse" as outras informações.</p>
...
Depois, apenas coloquei e configurei os títulos dos eixos da mesma forma que anteriormente, mas colocando o título do eixo x apenas no gráfico de baixo e o do eixo y apenas no do meio, já que não há necessidade de repeti-los por serem a mesma variável nos 3 gráficos.</p>

<p>A seguir, inseri o título de cada gráfico como o nome do país correspondente. Nesse caso, optei por alinhá-los à esquerda para que ficassem mais separados de todas as outras informações. Além disso, infelizmente, não há uma maneira (não encontramos, pelo menos) de colocar um título no gridplot em si. Portanto, ele acabou sem um título próprio.</p>

<p>Com relação ao fundo, novamente, a única alteração que fiz foi tornar os grids um pouco mais sutis.</p>

<p>A respeito das ferramentas, mantive nessa visualização as mesmas do segundo gráfico, apenas com a mudança de que configurei o hover para exibir o ano correspondente a cada barra e o seu valor da variação anual, o que avaliei como útil para melhorar o detalhamento das informações. Assim como antes, ativei o autohide e exclui a logo do Bokeh. Finalmente, mudei a posição da toolbar de cima para a lateral direita dos gráficos, pois a montagem já estava um pouco grande demais em altura.</p>
...
Além disso, inseri anotações em linha para deixar mais destacada a reta y = 0, de modo que é mais compreensível a localização da origem do gráfico.</p>

<p>Por fim, para montar o gridplot, optei por colocar os três gráficos um acima do outro, de modo a alinhar as marcações do eixo x. Fiz isso para que fossem mais facilmente comparáveis as informações ao longo dos anos.</p>
