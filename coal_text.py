...
first_graph = """ <center><h2>Comparação entre continentes da produção de energia a partir do carvão</h2></center>"""

text_grid_graph = """<p>O objetivo desse gráfico é criar uma comparação na evolução dos anos entre os continentes e seu consumo de energia primária a partir do carvão, os continentes escolhidos foram: América do Norte, Europa, América do Sul e Central e África.
...
title_stackedbar = """   <center><h2>Atividades do carvão</h2></center>"""

text_stackedbar = """<p>Essa visualização trás um acumulativo das atividades do carvão em alguns países no ano de 1995, entre os aspectos estão consumo, produção de carvão e geração de eletricidade. O tipo de mensagem desse gráfico é a comparação nominal, por isso foi escolhido a representação em barras que permite maior precisão na interpretação.</p>
...
title_rank = """<center><h2>Maiores consumidores de energia primária a partir do carvão (1965-2019)</h2></center>"""

text_rank = """<p>Nesse gráfico foi usado o Bokeh Server para permitr a interatividade do gráfico entre os anos disponíveis. A mensagem dessa visualização é um rank, assim as barras verticais foram escolhidas para representar os montantes de consumo.</p>
